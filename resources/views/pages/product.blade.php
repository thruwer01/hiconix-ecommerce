...
@section('description'){{$product->getMetaDescription()}}@endsection
@section('keywords'){{$product->getMetaKeywords()}}@endsection
@section('image')
...
<div class="container mb-5">
    <div class="" style="background-color: white;">
        ...
        <div class="product-name" style="display: flex;">
            <h1 class="fs-5">
				...
				@else
				<span style="font-size: 20px; color: #12aaed">{{$product->squere}}м²</span>
				@endif
                @endif
			</h1>
    ...
    <div class="row mt-3">
        <div class="col-lg-9 col-md-8 order-2 order-md-1">
            <div class="row">
				<div class="col-lg-8 mb-3 d-none d-md-block" style="position: relative;">
                    ...
                    <div style="position: absolute; columns: 1;">
						...
						<div style="position: relative; display: block; width: 64px; z-index: 999;" id="badgeNode" data-text="{{$badge['title']}}">
							@if (isset($badge['text_html']))
							{!!$badge['text_html']!!}
							@endif
							<img src="{{$badge['img']}}" style="display: block; z-index: 999;" width="64px" alt="{{$badge['title']}}" title="{{$badge['title']}}">
						</div>
						@endforeach
                        @endif
					</div>
                    @if ($product->status_new == "0")
                    <div class="img-is-archive" style="position: absolute; z-index: 9999; top: 15%; left: 32%; opacity: 0.7;">
                        <img src="/images/products/archive.png" width="200">
					</div>
                    @endif
                    <div class="product-img" style="position: relative;">
                        @if(!is_null($product->images()->first()))
                        <img src="https://erc.hiconix.ru/storage/{{$product->images()->first()->path}}{{$product->images()->first()->name}}.{{$product->images()->first()->extension}}"
            ...
            <div id="pinfo" class="mt-5">
                ...
                <div>{{$product->block_type->name}} <span class="color-main fw-bold">{{$product->model}}</span>@if($product->getParentCategory() == 26) нужно @else можно @endif скомплектовать со следующими @if($product->block_type->id === 1) внутренними @endif @if($product->block_type->id === 2) наружними @endif блоками @if ($product->block_type->id === 1)(входят в комплект):@endif</div>
                
                <div class="row products mt-3">
                    @foreach ($product->sets() as $set)
					<div class="col-12 col-sm-6 col-md-12 col-lg-6 col-xl-4">
						<div class="p-2 mb-3 border product-main" style="min-height: 400px; position:relative;">
							<div class="product-code mb-3">
								Код: {{$set->article}}
							</div>
							<a href="/products/{{$set->slug}}">
							<div class="pr-photo text-center"  style="min-height: 150px;">
								@if (!is_null($set->images()->first()))
								<img src="https://erc.hiconix.ru/storage/{{$set->images()->first()->path}}{{$set->images()->first()->name}}.{{$set->images()->first()->extension}}" style="max-height: 150px; max-width: 150px; max-height: 150px;">
								@else
								<img src="https://erc.hiconix.ru/images/products/no-photo.jpeg" style="max-height: 150px; max-width: 150px; max-height: 150px;">
								@endif
							</div>
							<div class="main-info pt-2">
								...
								<div style="font-size: 15px;" class="name">
									{{$set->brand->name}} {{$set->model}} {{$set->squere}} м²
								</div>
							</div>
							</a>
							<div class="product-buttons" style="font-size: 12px; display: flex; position: relative; justify-content: space-between; width: 100%;">
								...
								{!! number_format($set->ric_current, 0, " ", " ") !!} &#8381;
							</div>
								<div>
									<button data-product-id="{{$set->id}}" id="inCartBtn" class="p-2 px-4 in-cart active text-nowrap"><span>В корзину</span></button>
								</div>
							</div>
						</div>
					</div>
                ...
                <div>Данное доп.оборудование <span class="color-main fw-bold">{{$product->model}}</span> нужно скомплектовать со следующими блоками:</div>
                @else
                <div>{{$product->block_type->name}} <span class="color-main fw-bold">{{$product->model}}</span> нужно скомплектовать со следующим доп. оборудованием:</div>
                @endif
            ...
            <div class="sticky-top mt-3" style="top: 5rem; background-color:white;">
                <div class="btn-group" role="group">
                    ...
                    @if(count($product->docs()->get()) > 0)<a data-block-component="#documentation" class="btn px-3 border">Документация</a>@endif
            ...
            <div  class="product-full-info">
                <div class=mt-3 id="description">
                    ...
                    <div class="mt-3">
                        <h2 class="fs-5">Описание серии <span class="color-main">{{$product->series->name}}</span></h2>
                        <div class="mt-2" style="word-wrap: break-word; white-space: pre-line;">
                ...
                <div class="mt-3 col-8" id="properties">
					...
					<div class="product-properties mt-3">
						...
						<ul class="text-dark fs-10">
							...
							<li>
								<span class="prop-name">{{$props['property_name']}}@if(isset($props['property_units'])),@endif {{$props['property_units']}}</span>
                    ...
                    @endforeach
                    @if ($product->getParentCategory() == 26 && $product->block_type?->id == 2 && count($product->sets()) > 0 && count($product->sets()[0]->properties()) > 0)
                    <h5 class="mt-3">{{$product->sets()[0]->block_type->name}} <span class="color-main">{{$product->sets()[0]->model}}</span>:</h5>
					...
					<table class="table">
						...
						<tbody>
                            ...
                            <tr>
                                ...
                                <td>{!! round($document->size/1024/1024, 2) !!} МБ, {!!date('d.m.Y', strtotime($document->updated_at))!!}</td>
                                <td>
                                    <a href="https://erc.hiconix.ru/storage/{{$document->path}}/{{$document->name}}.{{$document->extension}}" target="_blank" class="btn-download p-2">Скачать</a>
								</td>
							</tr>
							@endforeach
						</tbody>
                ...
                <div class="mt-3" id="video">
                    <div class="row">
                        ...
                        <div class="col-6">
                            <div class="frame">
                                <iframe width="100%" height="250px" src="{{$video->getURL()}}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
							</div>
        ...
        <div class="col-lg-3 col-md-4 p-2 order-1 order-md-2">
            ...
            <div class="product-details--price border p-3 sticky-top" style="top: 100px;">
                ...
                <div class="products">
                    <span class="product-price fs-4" @if($product->style === "not_avaible")style="text-decoration: line-through;"@endif>
                        @if ($product->getParentCategory() == 26 && count($product->sets()) > 0 && $product->block_type->id == 2)
					{!! number_format($product->ric_current + $product->sets()[0]->ric_current, 0, " ", " ") !!} руб</span>
					@elseif ($product->getParentCategory() == 117 && count($product->sets()) > 0 && count($product->options_in_sets()) > 0 && $product->block_type->id == 2)
				{!! number_format($product->ric_current + $product->sets()[0]->ric_current + $product->options_in_sets()[0]->ric_current, 0, " ", " ") !!} руб</span>
			...
			<div class="text-muted" style="font-size: 14px;">
				...
				<ul class="list-unstyled">
					<li>код: {{$product->article}} <span class="fw-bold">{{$product->block_type->name}} {{$product->model}}</span></li>
					<li>код: {{$product->sets()[0]->article}} <span class="fw-bold">{{$product->sets()[0]->block_type->name}} {{$product->sets()[0]->model}}</span></li>
				</ul>
			</div>
			@elseif ($product->getParentCategory() == 117 && count($product->sets()) > 0 && count($product->options_in_sets()) > 0 && $product->block_type->id == 2)
			<div class="text-muted" style="font-size: 14px;">
				...
				<ul class="list-unstyled">
					<li>код: {{$product->article}} <span class="fw-bold">{{$product->block_type->name}} {{$product->model}}</span></li>
					<li>код: {{$product->sets()[0]->article}} <span class="fw-bold">{{$product->sets()[0]->block_type->name}} {{$product->sets()[0]->model}}</span></li>
					<li>код: {{$product->options_in_sets()[0]->article}} <span class="fw-bold">{{$product->options_in_sets()[0]->category->product_prefix}} {{$product->options_in_sets()[0]->model}}</span></li>
				</ul>
			</div>
			@endif
			<div>
				...
				<span class="text-muted" style="font-size: 16px;"><i class="fa-solid fa-circle-info text-danger"></i> Блоки продаются только в комплекте</span>
				@endif
			</div>
			<div class="text-center mt-3">
				@if ($product->status_new == 0)
				<button class="in-cart disabled px-5 py-2" disabled style="cursor: not-allowed; background-color: gray; color: white; border: gray;">В корзину</button>
				@else
